...
**Resultado real de la Tarea 5**: Brier Skill Score = +0.0193 en la temporada de test 2023-24 (nunca tocada durante entrenamiento ni calibración), contra +0.0031 del baseline Poisson en las mismas filas — una ventaja real, dentro del rango 0.01-0.05 que este plan definió de antemano como "victoria genuina" (no en la zona >0.08-0.10 que ameritaría sospecha de fuga de datos). La revisión final verificó independientemente toda la cadena de datos (priors.py → features.py → dataset.py → gbt_model.py → train_gbt.py) y no encontró ningún camino por el que datos de la temporada de test pudieran filtrarse al entrenamiento o la calibración.

**Matiz importante de la revisión final, a tener en cuenta antes de construir infraestructura en vivo sobre este resultado**: el marco "6x mejor que Poisson" debería abandonarse — el Poisson baseline (+0.0031) es del mismo orden de magnitud que el ruido (un modelo trivial que solo usa el minuto del partido ya da +0.0026 en las mismas filas), así que un cociente contra un número así no es informativo. El encuadre honesto es: "LightGBM muestra ventaja real; Poisson no muestra ninguna." Además, las 17.836 filas de test están agrupadas (13 cortes correlacionados por cada partido-equipo, ⁠así que la muestra efectivamente independiente es más cercana a 1.372), por lo que el resultado, aunque confiable como medición libre de fuga, todavía no tiene un intervalo de confianza estadístico ni una segunda repetición con otra temporada de test — ambas cosas son baratas de hacer (reusan el mismo pipeline) y son el paso recomendado antes de comprometerse a construir la Fase 2 sobre este modelo específico: (1) un bootstrap por partido para el intervalo de confianza del BSS, (2) reentrenar con una partición distinta (ej. 2223 como test) para confirmar que el resultado no es una casualidad de una sola temporada — esto es exactamente lo que demuestra "que sea repetible para otros partidos".

**Actualización — repetibilidad confirmada**: se corrió `src/goles/train_gbt_replication.py`, reentrenando con una partición cronológicamente honesta distinta (test=2022-23, validación=2021-22, entrenamiento=2018-19 a 2020-21, excluyendo 2023-24 por completo para simular de verdad "entrenar con el pasado"). Resultado: BSS=0.0167 (vs. 0.0193 en la corrida original), Poisson BSS=0.0023 (vs. 0.0031 original) — ambos números consistentes con la corrida original, mismo orden de magnitud, misma ventaja clara sobre Poisson, mismo ranking de features (poisson_prob/trailing_prior_xg/xg_diff/max_shot_xg dominan), calibración igual de buena. **La ventaja no es una casualidad de la temporada 2023-24** — se repite en una partición independiente. Con esto, la recomendación de repetición de la revisión final queda satisfecha; sigue pendiente el bootstrap de intervalo de confianza si se quiere mayor rigor estadístico, pero no es bloqueante.

Una vez confirmado con esa repetición, el modelo entrenado necesita un mecanismo de persistencia (guardar el booster + los parámetros de Platt scaling, ej. vía `booster.save_model()` y un JSON con `(a, b)`) antes de poder usarse en la inferencia en vivo de la Fase 2 — no se construyó acá, ya que dependía de saber primero si el modelo vale la pena persistir (ahora sabemos que sí). También quedan pendientes de la revisión: `fit_platt_scaling` no verifica si el optimizador de scipy convergió (`result.success`), lo cual podría devolver silenciosamente parámetros de calibración inválidos; y wiring ClubElo como señal independiente adicional (deferred desde la fase de fundamentos, ya que requiere emparejar nombres de equipos entre ClubElo y Understat). También deferred: guardas contra fechas vacías/no-ISO en `trailing_xg_per90` y una decisión documentada para cuando un partido anterior tiene cero tiros registrados (ambos señalados en la revisión final del plan de calibración como pendientes "antes de que `priors.py` toque datos en vivo").

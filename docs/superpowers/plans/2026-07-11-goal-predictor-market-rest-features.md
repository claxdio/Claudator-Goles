...
- Temporal discipline unchanged: market odds are *pre-match average odds* (`AvgH`/`AvgD`/`AvgA`/`Avg>2.5`/`Avg<2.5`, with `BbAv*` fallback for season 1819) collected before kickoff — NOT closing-line odds (football-data.co.uk separately publishes true closing-line columns, `AvgCH`/`AvgCD`/`AvgCA`/etc., not used here). Either way this is not look-ahead, since both are strictly pre-kickoff and a live system would have this same pre-match information available before/at kickoff. Note for future work: the `AvgC*` closing-line columns could be a follow-up experiment — closing lines reflect all pre-match information including late team-news and tend to be sharper than the earlier-collected average odds used here, which may be a weaker approximation of what a live system would actually see at kickoff. Red-card counts are computed with the same `minute <= cutoff_minute` discipline as shots — a card at minute 70 must never affect a cutoff-60 prediction. Test-season rows still never touch training or calibration.
...
Both training scripts re-ran cleanly against the fully-populated database (4,116 matches, market odds 4,116/4,116, 523 red-card events across 485 matches) after Tasks 1-8 landed.

| Run | Baseline BSS (post shot-enrichment) | New BSS | Δ |
|---|---|---|---|
| `train_gbt.py` (test 2324) | 0.0190 | **0.0337** | **+0.0147** |
| `train_gbt_replication.py` (test 2223) | 0.0160 | **0.0240** | **+0.0080** |

Both deltas are far outside the ±0.002 "flat" band, in the SAME direction, on both independent test seasons — this is a real, repeatable improvement, the largest of any plan in this project so far (roughly 75-80% relative improvement in BSS on both splits). Per the plan's decision rule, the new `FEATURE_NAMES` are **kept**, no revert needed.

**What drove it:** market odds, decisively. `own_market_wp` (the team's own no-vig market-implied win probability) is the single most important feature in the model by a wide margin in both runs — main run: 27,594.8 (4x the next feature); replica: 21,128.2 (3.9x the next feature). The whole market-odds category (`own_market_wp`, `opp_market_wp`, `market_over25_wp`, `market_draw_wp`) occupies 4 of the top 5 slots in both runs. This confirms the research hypothesis exactly: bookmaker-implied probability compresses private information (injuries, form, sharp money) that no on-pitch statistic can see, and it was genuinely orthogonal to everything already in the model — unlike the shot-enrichment plan's box/set-piece features, which came back flat because they were redundant with xG.

**Red cards contributed real, secondary signal**, not dominant but consistently present: `opp_red_cards` ranks #10 in the main run (1,900.3) and #13 in the replica (1,665.0) — meaningfully above the median feature. Interesting asymmetry in both runs: the *opponent's* red card matters more than *own* red card (`own_red_cards` ranks #16 / #23) — plausible, since going a man up is a clearer goal-scoring opportunity than going a man down is a clear goal-conceding risk (a well-organized 10 men can still defend well for a while).

**Rest-days contributed modest, real signal** (`opp_rest_days`/`own_rest_days` land in the middle third of both rankings, roughly rank #17-21) — present but not a major driver.

**Calibration remains excellent**: both runs show pred≈real agreement across the two lowest bins (which hold >99% of predictions in both runs), and the [0.4-0.6) bin (small sample, n=197/277) is now the only place with a visible gap (underconfident, i.e. the safe direction) — same pattern as before, unaffected by the new features.

The persisted model (`data/model/booster.txt` + `platt.json`, from the earlier model-persistence plan) was automatically regenerated by this run and now reflects the full 36-feature model — no separate persistence step needed.

**Update — final whole-branch review and fix.** The final review (fable model) traced the full leakage path and found no issue with market odds, rest-days, or the temporal-holdout discipline, but found one real Critical defect: `load_red_cards_from_cache` used a substitute's raw `time` field directly as their dismissal minute, when for substitutes that field is actually *minutes played after coming on* (a duration), not a match-clock minute — confirmed concretely (Eddie Nketiah, match_id 11973: recorded at minute 4, really dismissed around minute 75-79). This affected 60 of 523 red-card events (11.5%). Fixed by reconstructing the true minute via the substitute's `roster_out` cross-reference to the replaced player's own `time` field (= substitution-on minute) plus the substitute's own `time` (= minutes played after that) — verified against the real cache, with all 59 affected cases resolving cleanly (0 dropped). Also corrected: the plan's "closing lines" wording (the odds used are pre-match average odds, not true closing lines — see the corrected Global Constraints above), a fail-loud check in `load_match_cards`, an `opp_rest_days` loop-hoisting/consistency cleanup, and two dead-import removals.

Both scripts were re-run after the fix:

| Run | Pre-fix BSS | Post-fix BSS | Δ |
|---|---|---|---|
| `train_gbt.py` (test 2324) | 0.0337 | **0.0335** | -0.0002 |
| `train_gbt_replication.py` (test 2223) | 0.0240 | **0.0237** | -0.0003 |

Both changes are an order of magnitude smaller than the reviewer's predicted ±0.002 tolerance — the headline result is confirmed, not an artifact of the mistimed-substitute bug. `opp_red_cards`'s feature importance actually *increased* with the fix (main: 1,900→2,445, rank #10→#7; replica: 1,665→2,144, rank #13→#8), consistent with a real feature carrying cleaner signal once correctly timed. **Final, verified numbers: BSS 0.0335 (main) / 0.0237 (replica), both against a 0.0190/0.0160 baseline — a confirmed, repeatable, ~75%/~48% relative improvement.**

## Próximos pasos (fuera de alcance de este plan)

Remaining from the GBT plan's backlog: bootstrap confidence interval (optional hardening, less urgent now given the effect size here is much larger than the earlier borderline result). ClubElo wiring is now lower priority — market odds already provide a strong pre-match team-strength-equivalent signal with less engineering risk. If red cards' value grows with more data, Phase 2's live pipeline needs a live source for card events with minute timestamps (Sofascore/FotMob both surface cards live, typically with minute — needs verification when Phase 2 is scoped) since Understat's raw cache obviously has no live equivalent. Market odds ARE available live (this is literally what Phase 2's Betfair/odds-comparison integration was always going to fetch) — this result raises the priority of getting that live odds pipeline right, since it's now demonstrably the single most valuable signal in the whole model.

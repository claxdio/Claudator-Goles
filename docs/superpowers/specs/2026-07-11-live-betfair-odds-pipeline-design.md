...
## Actualización: bloqueo geográfico y proxy UK

Once real credentials were entered, the first live login failed with `Betfair login failed: BETTING_RESTRICTED_LOCATION`. Root cause (verified, not assumed): the VPS's IP (`85.239.245.73`) geolocates to St. Louis, Missouri, US — Betfair Exchange is fully blocked for US-geolocated IPs at both the login endpoint and the data endpoints, regardless of account origin.

**Fix:** rather than move the whole VPS (shared with the unrelated `anjuma` project) or pay for new infrastructure, a free-tier Oracle Cloud "Always Free" VM (`VM.Standard.E2.1.Micro`, Ubuntu, region `UK South (London)`) was provisioned as a dedicated egress point, confirmed via `ipinfo.io` to geolocate to London, GB. The existing VPS opens a self-healing SSH dynamic port forward (`ssh -N -D 1080`) to this VM, and all Betfair traffic (both `cert_login` and subsequent API calls) is routed through the resulting local SOCKS5 proxy via a new optional `proxy_url` parameter on `BetfairSession`/`cert_login` (`src/goles/betfair/auth.py`). The Docker entrypoint (`docker-entrypoint-betfair.sh`) starts the tunnel (with auto-reconnect) before execing the poller; `openssh-client` was added to `Dockerfile.betfair`, and `PySocks` to `pyproject.toml` (required by `requests` for `socks5h://` proxy URLs). New env vars: `UK_PROXY_HOST`, `UK_PROXY_USER`, `UK_PROXY_KEY_FILE`, `BETFAIR_SOCKS_PROXY_URL`. The UK VM's dedicated SSH private key lives only on the VPS host (`/root/goles-uk-proxy-key/id_ed25519`, bind-mounted in), never in git.

**Verified working end-to-end in production:** after redeploying with the proxy wired in, the Dokploy Logs tab showed the SSH tunnel connecting, then `"24 mercados encontrados en las ligas trackeadas"` (login succeeded, no more geo-block). Querying `live_odds.db` directly inside the running container confirmed 72 real snapshot rows within the first two poll cycles, with real fixtures (e.g. Bundesliga pre-season matches: Mainz v Paderborn, Union Berlin v Eintracht Frankfurt, Freiburg v Werder Bremen, Elversberg v Leverkusen) and computed no-vig probabilities for both `MATCH_ODDS` and `OVER_UNDER_25` markets. **The live-odds pipeline is fully operational.**
